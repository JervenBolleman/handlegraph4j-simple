...
public class CompressedBufferedChunk<T> implements Chunk<T> {
    ...
    public CompressedBufferedChunk(CompressedChunk<T> from,
    ...
    public CompressedBufferedChunk(ByteBuffer buffer,
            LongLongToObj<T> reconstructor,
            ToLong<T> getKey
    ) {
        this.buffer = buffer;
    ...
    private static ByteBuffer intoBuffer(int size, int[] compressedKeys, int[] compressedValues, int firstKey, int firstValue, int lastKey, int lastValue) {
