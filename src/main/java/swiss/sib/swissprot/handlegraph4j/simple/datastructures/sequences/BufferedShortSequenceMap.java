...
public class BufferedShortSequenceMap implements NodeSequenceMap {
	...
	public BufferedShortSequenceMap(RandomAccessFile raf) throws IOException {
		...
		for (int i = 0; i < noOfsequences; i++) {
			...
			for (int j = 0; j < maps; j++) {
				...
				MappedByteBuffer map = raf.getChannel().map(MapMode.READ_ONLY, raf.getFilePointer(), bits);
	...
	static void fillLengthBitMaps(Map<Integer, List<RoaringBitmap>> lengthBitmaps,
			Map<Sequence, List<ImmutableRoaringBitmap>> fewNps) {
	...
	public static void writeToDisk(ShortSequenceMap ssm, DataOutputStream raf)
	...
	private static void sequenceToLong(Sequence seq, DataOutputStream raf) throws IllegalStateException, IOException {
	...
	private Sequence readSequence(RandomAccessFile raf) throws IllegalStateException, IOException {
	...
	@Override
	public long size() {
		return fewNps.values().stream().flatMap(List::stream).mapToLong(ImmutableRoaringBitmap::getLongCardinality)
				.sum();
	}
	...
	private class SequenceAndRoaringBitMap implements
		...
		@Override
		public AutoClosedIterator<NodeSequence<SimpleNodeHandle>> apply(
				Map.Entry<Sequence, List<ImmutableRoaringBitmap>> en) {
